""" Class with preprocessing functions only used by GAUR to create Hybrid models.
"""
import pandas as pd
import re
import logging
...
def _get_li_features_from_query(s: pd.Series) -> pd.Series:
...
def pre_process_for_li(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Copy shouldn't be needed here, already done in caller. But uncomment if
    # 
    # _df = df.copy()
    _df = df.apply(_get_li_features_from_query, axis=1)
    return _df
