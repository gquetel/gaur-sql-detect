...
from xpgaur.models.Gaur import AutoEncoder_Gaur, LOF_Gaur, OCSVM_Gaur
...
def train_lof_gaur(
    ...
    df_traces = get_traces_from_df(
        ...
        use_datadir=args.use_datadir,
    )

    # We merge the traces with the original DataFrame to get all column informations.
    # Then we retrive the train, test and val sets.
    # We only keep entries which returned a trace, a CRITICAL message is displayed
    # before that to warn of an error for each missing trace.
...
def train_ocsvm_gaur(
    ...
    # care of that.
    df_traces = get_traces_from_df(
        df=df, use_cache=args.use_cache, use_datadir=args.use_datadir
    )
...
def train_ae_gaur(
    ...
    # We merge the traces with the original DataFrame to get all column informations.
    # Then we retrieve the train, test and val sets.
...
def train_models(
    ...
    for trace in traces:
        ...
        xpgaur.update_location_mysqlfiles(trace)
    ...
    xpgaur.update_location_mysqlfiles("expert")
